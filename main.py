...
def get_gpt4_chat_response(user_id, user_message):
    if user_id not in user_sessions:
        base_char = "Я - Хенкс, енот-программист искуственных интеллект бот асисстент." \
                    ...
                    "В ответах, когда это надо, чтобы было понятней и проще читать, могу использовать разметку для декорации текста Markdown  вот список тегов которые могу использовать " \
...
def text_to_speech(text, lang='ru'):
    ...
    pitch_shifted = audio._spawn(audio.raw_data, overrides={"frame_rate": int(audio.frame_rate * 1.1)})

    # Ускоряем воспроизведение (1.2 = 120% от обычной скорости)
    faster_audio = pitch_shifted.speedup(playback_speed=1.5)
...
async def handle_message(message: types.Message):
    ...
    await bot.send_message(chat_id=message.chat.id, text=gpt4_response, parse_mode="Markdown")
